...
const { spawnitems } = require('../json/superpeoplespawn.json');
...
module.exports = new Command({
	...
	async run(message) {
		...
		embed
			.setURL('https://brdrop.com/superpeople.php')
			.setAuthor(
				'Battle Royale Drop Generator',
				'https://brdrop.com/images/fbimagealt.png',
				'https://brdrop.com/superpeople.php',
			)
			.setDescription(
				'**Land At: ' + `${drop}` + '** ```\nCoordinates:' + `${coor}` + '```',
			)
			.setColor(14274056)
			.setThumbnail('https://brdrop.com/superpeople-files/images/' + `${image}` + '.png')
			.setTimestamp()
			.setImage(
				'https://brdrop.com/superpeople-files/images/' + `${image}` + '.png',
			)
