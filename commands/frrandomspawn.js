...
const { spawnitems } = require('../json/frrandomspawn.json');
...
module.exports = new Command({
	...
	async run(message) {
		...
		embed
			.setURL('https://brdrop.com/fortnite.php')
			.setAuthor(
				'Battle Royale Drop Generator',
				'https://brdrop.com/images/fbimagealt.png',
				'https://brdrop.com/fortnite.php',
			)
			.setDescription(
				'**Land At: ' + `${drop}` + '** ```\nCoordinates:' + `${coor}` + '```',
			)
			.setColor(14274056)
			.setThumbnail('https://brdrop.com/fortnite-files/images/' + `${image}` + '.png')
			.setTimestamp()
			.setImage(
				'https://brdrop.com/fortnite-files/images/' + `${image}` + '.png',
			)
