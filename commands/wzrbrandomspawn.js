...
const { spawnitems } = require('../json/wzrbrandomspawn.json');
...
module.exports = new Command({
	name: 'wzrbdrop',
	description: 'Randomized Drop Generator for Warzone Rebirth',
	usage: 'Display Warzone Rebirth Drop',
	type: 'BOTH',
	slashCommandOptions: [],
	...
	async run(message) {
		...
		embed
			.setURL('https://brdrop.com/warzone-rebirth.php')
			.setAuthor(
				'Battle Royale Drop Generator',
				'https://brdrop.com/images/fbimagealt.png',
				'https://brdrop.com/warzone-rebirth.php',
			)
			.setDescription(
				'**Land At: ' + `${drop}` + '** ```\nCoordinates:' + `${coor}` + '```',
			)
			.setColor(14274056)
			.setThumbnail('https://brdrop.com/warzone-rebirth-files/images/' + `${image}` + '.png')
			.setTimestamp()
			.setImage(
				'https://brdrop.com/warzone-rebirth-files/images/' + `${image}` + '.png',
			)
