...
const { spawnitems } = require('../json/arandomspawn.json');
...
module.exports = new Command({
	name: 'aldrop',
	description: 'Randomized Drop Generator for Apex Legends',
	usage: 'Display Apex Legends Drop',
	type: 'BOTH',
	slashCommandOptions: [],
	...
	async run(message) {
		...
		embed
			.setURL('https://brdrop.com/apexlegends.php')
			.setAuthor(
				'Battle Royale Drop Generator',
				'https://brdrop.com/images/fbimagealt.png',
				'https://brdrop.com/apexlegends.php',
			)
			.setDescription(
				'**Land At: ' + `${drop}` + '** ```\nCoordinates:' + `${coor}` + '```',
			)
			.setColor(14274056)
			.setThumbnail('https://brdrop.com/apexlegends-files/images/' + `${image}` + '.png')
			.setTimestamp()
			.setImage(
				'https://brdrop.com/apexlegends-files/images/' + `${image}` + '.png',
			)
